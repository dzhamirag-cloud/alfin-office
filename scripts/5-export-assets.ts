...
// Track used filenames per category to deduplicate
const usedNames = new Map<string, number>();

for (const asset of assets) {
  const categoryDir = join(assetsDir, asset.category);

  // Deduplicate filenames: append _2, _3, etc. for duplicates
  const nameKey = `${asset.category}/${asset.name}`;
  const count = (usedNames.get(nameKey) || 0) + 1;
  usedNames.set(nameKey, count);
  const dedupName = count === 1 ? asset.name : `${asset.name}_${count}`;

  const filename = `${dedupName}.png`;
