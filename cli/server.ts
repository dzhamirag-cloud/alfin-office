 ...
 * Plain Node.js http.createServer — no Express needed.
 */
...
export function createServer(webviewDir: string, orchestrator: CliOrchestrator): http.Server {
  const server = http.createServer((req, res) => {
    ...
    if (!resolved.startsWith(path.resolve(webviewDir))) {
      res.writeHead(403);
      res.end('Forbidden');
      return;
    }
